...
class Briscola_env:
    def __init__(self):
        ...
        self.cards_in_the_deck = np.array(cards_in_string)
    ...
    def determine_points(self, card_player_1, card_player_2, order, passed_briscola = None):
        '''
        order == True -> card_first_player is played first
        
        Determines (1) who wins a round and (2) how many points they win.
        
        Returns: player_1_wins, points_winner
        Updates: nothing
        '''
        #Determines who wins and how many points
        [first_card, second_card] = [card_player_1, card_player_2] if order else [card_player_2, card_player_1]
        winner = self.which_card_wins_a_round(first_card, second_card, passed_briscola)
        points = my_cards.loc[first_card[0], 'points'] + my_cards.loc[second_card[0], 'points']
        played_cards = (first_card, second_card)
        
        player_1_wins = winner == 'first_played_card' if order else not winner == 'first_played_card'
        
        return player_1_wins, points
