...
### Play and record match in df



dataframe_index = ['is first to play', 'pl 1 hand 1', 'pl 1 hand 2', 'pl 1 hand 3', 'pl 2 hand 1', 'pl 2 hand 2', 'pl 2 hand 3', 'briscola', 'pt player 1', 'pt player 2',  'played card', 'opponent card'] + list(range(40))
...
def play_first_round_for_prob(player_1, player_2, partita, epsilon = 0.1):
    ...
    Returns:a pd.DataFrame with column 0 and the following indices:
    who plays first, which card each player plays, each player's hands, each player's points.
    
    new_matches_df is a pd.DataFrame of previous matches.
    ...
    my_index = dataframe_index
    new_matches_df = pd.DataFrame({0: current_state}, index = my_index)
...
def play_a_round_for_prob(player_1, player_2, partita, new_matches_df, number_of_previous_games = 0, epsilon = 0.1):
    '''
    ...
    Returns the pd.df new_matches_df.
    ...
    new_matches_df[21*number_of_previous_games + 20 - partita.round] = current_state
    ...
    if partita.round == 0:
        ...
        new_matches_df[20 +  21*number_of_previous_games] = final_state
...
def simulate_games_and_record_data(number_of_simulations, player1, player2, partita, epsilon = 0.1, pl_1_model = None, pl_2_model = None):
    ...
    while partita.round > 0:
        play_a_round_for_prob(player_1, player_2, partita, new_matches_df, 0, epsilon)
    partita.reset()
    ...
    for _ in range(1, number_of_simulations):
        ...
        while partita.round > 0:
            play_a_round_for_prob(player_1, player_2, partita, new_matches_df, number_of_previous_games =  _, epsilon = epsilon)
            
        partita.reset()
    return new_matches_df.transpose().fillna(value = 'None').to_numpy() 
...
def get_points(my_games):
    ...
    reshaped_pts = np.reshape(my_points, (int(my_points.shape[0]/21), 21, 2))
...
def simulate_games(number_of_simulations, player1, player2, partita, pl_1_model = None, pl_2_model = None, vs_human = False):
    '''
    simulates number_of_simulations matches between player1 and player2
    ...
    for _ in range(1,number_of_simulations):
        ...
        coin_flip_player_1_is_first = coin_flip()
        player_1.new_game(first_hand[0], briscola, is_first_player = coin_flip_player_1_is_first)
        player_2.new_game(first_hand[1], briscola, not coin_flip_player_1_is_first)
